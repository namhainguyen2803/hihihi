#!/usr/bin/env python3
""" Calculates the Frechet Inception Distance (FID) to evalutate_fid GANs.
...
def _handle_path(path, sess, low_profile=False,batch_size=100):
    if path.endswith('.npz'):
        ...
        if low_profile:
            m, s = calculate_activation_statistics_from_files(files, sess,batch_size=batch_size)
        else:
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False,batch_size=100):
    ...
    # from evalutate_fid import memory
    # memory()
